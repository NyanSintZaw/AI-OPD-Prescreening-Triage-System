...
i18n: i18next with inline resources in `src/i18n/resources.ts` — exactly two languages (`th` default, `en`); update both blocks when adding strings. Styling is plain global CSS on the **MALI design system** (Brand v1.0), vendored from the `@notuning/mali-ds` package (`~/Projects/mali-design-system`, authored in Claude Design) into `src/design-system/` — tokens, component CSS, and the brand marks (`NongMali` is the main logo; `Mark` is the bud, used for loading/progress). `src/styles/tokens.css` is just the entry that imports it, and `src/design-system/aliases.css` is a migration shim re-pointing retired `--mch-*` names at MALI tokens. Edit the package and re-copy — never hand-edit `src/design-system/`. See `hospital-hotline-assistant-web/DESIGN.md`. No Tailwind/CSS Modules. `RecommendationCard.tsx` renders triage results and embeds the static wayfinding map from `public/hospital-map/` via iframe.
