...
| `assessment` | No — **composed at send time** | the triage level and pain score. The level is here only because your contract has no field for it — see change request 1. **The vitals are deliberately not restated here**; they travel as numbers, once — see below. |
...
| `vitals` (structured, with `measured_at` and a per-field `sources` map) | CR 2 | **Provenance is per field, not per reading.** The BP and pulse come off an Omron cuff; the weight, height and often the temperature are what the patient told us. Treating them as one kind of evidence would let a self-reported weight be charted as a booth measurement. We send each reading **once**, as a number, rather than also restating it in `sbar.assessment` — the same value in two shapes drifts the moment one is edited, and a sentence cannot carry provenance you can query. These are the only vital fields we hold; if the shape does not suit you, tell us and we will match yours. |
...
**CR 8 — an idempotency key we cannot query leaves us blind.** Your contract
documents one endpoint, `POST /patient-assignments`, so there is no way to
read an assignment back. If our call times out, the only way to find out
whether it landed is to send it again and interpret the answer. A read
endpoint turns a guess into a fact, and enables end-of-day reconciliation
against your gateway logs. Largely redundant if CR 7 is granted.
