...
- Routes live in feature routers under `app/routers/` (shared auth deps in `routers/deps.py`); `app/main.py` only wires lifespan/services + `include_router`. Raw asyncpg SQL via `app/database.py` — no ORM. Migrations = numbered raw SQL in `migrations/`, applied by `scripts/init_db.py` (idempotent, tracks `schema_migrations`). Editing `app/data/screening_criteria.json` changes the EVALS ONLY — the engine reads the active DB row. Publish with `scripts/deploy_criteria.py` (dry-run by default, `--activate` to publish).
