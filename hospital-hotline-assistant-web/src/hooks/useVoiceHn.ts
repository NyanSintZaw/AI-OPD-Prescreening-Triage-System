...
export type VoiceHnState = 'idle' | 'recording' | 'processing';
...
/**
 * Extract a numeric HN from a speech transcript. Handles Arabic digits
 * ("099 000 01"), Thai numerals (๐๙๙…), and Thai spoken number words
 * ("ศูนย์ เก้า เก้า…"). Everything non-numeric is dropped.
 */
...
/**
 * One-shot microphone capture for the HN step. Tap to start, tap to
 ...
 * streaming pipeline.
 */
export function useVoiceHn(language: AppLanguage) {
  const [state, setState] = useState<VoiceHnState>('idle');
  ...
  const stop = useCallback(async (): Promise<string> => {
    ...
    cleanupStream();
    try {
      const result = await api.stt(audioBlob, language, 'hn.webm');
