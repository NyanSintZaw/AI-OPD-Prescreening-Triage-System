...
`nongShowreel` (the mixer — explode, heartbeat and rise-sway in a random order, with the repeat counts varied too, and never the same act twice in a row, so a passer-by never sees a fixed loop), `nongExplode` (fly apart & snap — she breaks into her parts, which drift outward and hover in an exploded view, bobbing gently, then snap back together with a ring burst and a petal splash), `nongHeartbeat` (heartbeat burst — a lub-dub double pulse every beat, each throwing off expanding rings and a scatter of jasmine petals; a medical rhythm made friendly).
