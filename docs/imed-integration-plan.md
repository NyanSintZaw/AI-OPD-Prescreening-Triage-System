...
| 7 | **Clarify first:** does a 409 `VISIT_QUEUE_ALREADY_EXIST` carry `result`? Your Response Definition documents `result.queue_number`, but the Error responses section says errors return only `status` + `message` + `message_th`. Both readings fit the document. **If errors carry no `result`**, then: return 200 with the original result on a duplicate `request_id`, or include `result` in the 409 body | After a timeout we retry with the same `request_id` and get "already exists". The patient *is* queued — but if that response has no `result`, we never learn `queue_number` and the nurse has nothing to hand them. Our adapter already handles both shapes, so nothing breaks either way; this decides whether the nurse gets the number or has to look it up in iMed. |
