...
- One accent per screen. Gold (`--color-accent`) is a signature: focus ring, active step, wordmark dot, Orb halo, at most one small element. Never a button fill, never a background, never a large area.
- Primary actions are ink (`--color-primary`, near-black). Hierarchy comes from weight and size, not from extra colours.
...
- No gradients. The only exception is the Orb halo, which is a token (`--shadow-gold-halo`).
