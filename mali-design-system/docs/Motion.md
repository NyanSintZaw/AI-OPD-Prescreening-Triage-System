...
category: Brand
---
The approved logo motions, ported from the Brand Guidelines canvas. Pass the `motion` prop to `<Mark>` or `<NongMali>` to play one on mount, or call `playMark(element, name)` to trigger one yourself. Every call resets the mark first, so replaying mid-flight is safe, and each returns a handle with `cancel()`.
...
`nongWaveHello` (wave hello — she leans toward the patient and waves her side petal while bobbing, straightens, does a squash-and-stretch hop, then sways until she waves again; the most welcoming loop for greeting people at the booth), `nongShowreel` (the mixer — wave hello, heartbeat and sway in a random order, with the repeat counts varied too, and never the same act twice in a row, so a passer-by never sees a fixed loop), `nongBounce` (playful bounce — she drops in from above, lands with a squash, wiggles hello, then spin-jumps and settles; the most character-forward loop, and the only one with an entrance, which is why the showreel leaves it out — a drop-in mid-loop reads as her arriving all over again), `nongExplode` (fly apart & snap — she breaks into her parts, which drift outward and hover in an exploded view, bobbing gently, then snap back together with a ring burst and a petal splash), `nongHeartbeat` (heartbeat burst — a lub-dub double pulse every beat, each throwing off expanding rings and a scatter of jasmine petals; a medical rhythm made friendly).

**Her face.** The attract loops animate her eyes and mouth as well as her body — they are found by their deep-teal ink (`#2C7978`) and ordered down the face, so no path is hand-numbered and a re-drawn mark still works. Each act sets its own expression: eyes scrunch shut on a landing and pop wide on the rebound, a double blink and a held smile carry the wave, and under the heartbeat and the sway she just stays alive — a lazy blink at a slightly different moment each turn, sometimes doubled, with a soft smile on roughly half of them. The randomness is rolled per beat rather than on a timer of its own, so every act still ends when it says it does and the showreel can chain them.

These also draw **outside** the SVG: expanding rings and petal particles are DOM nodes added to the mark's parent, so the parent wants to be roughly twice the mark's size and is temporarily made `position: relative` if it is `static` (restored on cancel). Pass `{ stage }` to draw them somewhere else. Everything spawned is removed when the handle is cancelled.
